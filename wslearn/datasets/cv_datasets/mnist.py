...
class Mnist(Dataset):
    ...
    def __init__(self, lbls_per_class, ulbls_per_class=None, seed=None,
        ...
        X_tr_lb, y_tr_lb, X_tr_ulb, y_tr_ulb = split_lb_ulb_balanced(X=X_tr, y=y_tr, lbls_per_class=lbls_per_class, ulbls_per_class=ulbls_per_class, seed=seed)

        if not return_ulbl_labels:
            y_tr_ulb = None

        self.lbl_dataset = TransformDataset(X_tr_lb, y_tr_lb,
                                            transform=self.transform,
                                            weak_transform=self.weak_transform,
                                            strong_transform=self.strong_transform)


        self.ulbl_dataset = TransformDataset(X_tr_ulb, y_tr_ulb,
                                            transform=self.transform,
                                            weak_transform=self.weak_transform,
                                            strong_transform=self.strong_transform)
