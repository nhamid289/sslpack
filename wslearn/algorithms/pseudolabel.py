...
class PseudoLabel(Algorithm):
    ...
    def __init__(self, lambda_u=0.5, conf_threshold=0.95, max_pseudo_labels=None,
        ...
        Args:
            lambda_u: A number between 0 and 1 denoting the weight of the
                unlabelled loss in the total loss.
            ...
            sup_loss_func: a function with signature f(pred, true) to compute
                the loss on the supervised batch. Cross entropy by default
            unsup_loss_func: a function with signature f(pred, true, mask) to
                compute the loss on the unsupervised batch. Masked cross entropy
                by default
        """
